...
def _report_payload(text: str, include_events: bool) -> dict[str, Any]:
    parsed = parse_log_document(text)
    if not parsed.events:
        raise CLIError("Input contains no non-empty log events")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        payload = _report_payload(text, include_events=not args.omit_events)
